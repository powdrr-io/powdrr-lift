...
The required final package surfaces are `powdrr_lift.product` for the product
and lifecycle language, `powdrr_lift.process` for the LLM-led process language,
and `powdrr_lift.agent` for proposal, provider, and runner APIs. The existing
...
- `powdrr_lift.product`, `powdrr_lift.process`, and `powdrr_lift.agent` are
