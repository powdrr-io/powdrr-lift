...
1. Create the template.
   - Run `powdrr-lift system-specification`.
   - If using MCP, call `create_system_specification`.
   - Use the default file at `docs/system/system-specification.yaml` unless the task calls for a different path.
...
3. Validate the specification.
   - Run `powdrr-lift evaluate-system-specification`.
   - If using MCP, call `validate_system_specification`.
