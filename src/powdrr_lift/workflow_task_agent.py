...
def _task_system_prompt() -> str:
    return (
        ...
        "Task input contract: input_state contains the declared inputs for this "
        "task. When present, input_state.upstream_task_outputs is the durable "
        "contract from completed upstream tasks. Each entry is keyed by task id "
        "and contains output_state_type and output_state. Treat those outputs as "
        "the source of truth for filling the declared inputs; do not invent or "
        "silently rename upstream values. Your complete action output_state must "
        "satisfy this task's output_state_type and be sufficient for every "
        "downstream task that declares this task as an upstream dependency.\n"
