...
from powdrr_lift.workrr.action_protocol import (
...
from powdrr_lift.workrr.chat_actions import (
...
from powdrr_lift.workrr.chat_context import (
    _load_workflow_context,
    _resolve_worktree_context,
    _resolve_worktree_for_request,
    _worktree_reuse_decision,
)
from powdrr_lift.workrr.chat_contract import (
    _action_repair_prompt,
    _step_action_response_schema,
    _workflow_edit_failure_feedback,
)
from powdrr_lift.workrr.chat_io import _prompt_user
from powdrr_lift.workrr.chat_selection import (
    SkillChatConfig,
...
from powdrr_lift.workrr.chat_transport import (
...
from powdrr_lift.workrr.execution_state import (
...
def test_failed_evaluator_gate_exposes_structured_issues_for_repair(
    ...
    monkeypatch.setattr(
        "powdrr_lift.workrr.execution_loop._execute_shell_tool",
...
def test_oversized_context_uses_long_context_backup_model(
    ...
    monkeypatch.setattr(
        "powdrr_lift.workrr.chat_transport.model_limits_for",
...
def test_workflow_execution_terminalizes_unrepairable_actions(
    ...
    monkeypatch.setattr(
        "powdrr_lift.workrr.chat_context._resolve_worktree_context",
...
def test_run_workflow_chat_surfaces_current_file_context_for_edit_actions(
    ...
    monkeypatch.setattr(
        "powdrr_lift.workrr.chat_agent.subprocess.run",
        _fake_run,
    )
    monkeypatch.setattr(
        "powdrr_lift.workrr.chat_context._resolve_worktree_context",
...
def test_run_workflow_chat_prompts_for_retry_on_provider_failure(
    ...
    monkeypatch.setattr(
        "powdrr_lift.workrr.chat_agent.time.sleep",
        sleep_calls.append,
    )
...
def test_run_workflow_chat_executes_shell_tool_actions(
    ...
    monkeypatch.setattr("powdrr_lift.workrr.chat_agent.subprocess.run", _fake_run)
...
def test_execute_shell_tool_does_not_double_wrap_rtk(
    ...
    with patch("powdrr_lift.workrr.chat_agent.subprocess.run") as run:
...
def test_execute_shell_tool_falls_back_when_rtk_is_unavailable(
    ...
    monkeypatch.setattr("powdrr_lift.workrr.chat_agent.shutil.which", lambda _: None)
    with patch("powdrr_lift.workrr.chat_agent.subprocess.run") as run:
        run.return_value.returncode = 0
        run.return_value.stdout = ""
        run.return_value.stderr = ""
